...
export const resetUnitOrder = (s: UnitSelection, a: UnitSelectionAction): UnitSelection => {
    const { newUnitOrder } = a
    const { orderedUnitIds } = s
    if (!newUnitOrder || newUnitOrder.length === 0) throw Error('Attempt to reset unit ordering to empty set.')
    const oldUnits = new Set<number | string>(orderedUnitIds)
    if (oldUnits.size > 0 && (newUnitOrder.length !== oldUnits.size || newUnitOrder.some(id => !oldUnits.has(id)))) {
...
export const updateSort = (s: UnitSelection, a: UnitSelectionAction): UnitSelection => {
    const { sortRules } = s
